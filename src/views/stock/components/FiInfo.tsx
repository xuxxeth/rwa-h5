...
const Financials = memo(() => {
  ...
  const [current2, setCurrent2] = useState('pb')
  const [current3, setCurrent3] = useState('ocfps')
  ...
  const filterData2 = useMemo(() => {
    const key = current2.toLowerCase()
    const annualKey = (key + '_annual') as string
    // yoy 是 Year-Over-Year 的缩写，用于表示同比增长率
    const yoyKey = (annualKey + '_yoy') as string
    ...
    return _data
  }, [current2, indicatorsData])

  const filterData3 = useMemo(() => {
    const key = current3.toLowerCase()
    const annualKey = (key + '_annual') as string
    // yoy 是 Year-Over-Year 的缩写，用于表示同比增长率
    const yoyKey = (annualKey + '_yoy') as string
    ...
    return _data
  }, [current3, indicatorsData])

  const roeName = useMemo(() => {
    return annualList.find(item => item.value === current)?.label
  }, [current, annualList])

  const roeName2 = useMemo(() => {
    return annualList.find(item => item.value === current2)?.label
  }, [current2, annualList])

  const roeName3 = useMemo(() => {
    return annualList.find(item => item.value === current3)?.label
  }, [current3, annualList])
  ...
  const roeFormula2 = useMemo(() => {
    if (['ocfps', 'grps'].includes(current2)) {
      return undefined
    }
    return t(`financials.formulas.${current2}`)
  }, [current2, t])

  const roeFormula3 = useMemo(() => {
    if (['ocfps', 'grps'].includes(current3)) {
      return undefined
    }
    return t(`financials.formulas.${current3}`)
  }, [current3, t])

  return (
    <div className='p-4 bg-[#0E0F12] rounded-[4px] mt-2 pb-[100px]'>
      ...
      <div>
        ...
        <div className='w-full h-[160px] rounded-2xl' tabIndex={-1}>
          <ResponsiveContainer width='100%' height='100%'>
            <ComposedChart
              data={filterData}
              margin={{ top: 5, right: 0, bottom: 20, left: 0 }} // ✅ 给够空间
              barCategoryGap='25%'
              barGap={6}
            >
              <CartesianGrid
              ...
              <YAxis
                ...
                tick={({ x, y, payload }) => (
                  <text x={x + 70} y={y} textAnchor='end' fill='#737A87' fontSize={12} dy={4}>
        ...
        <div className='text-[#9DA3AF] text-center text-xs/[15px] h-[15px] font-normal mt-4'>{roeFormula}</div>
      </div>
      <div className='mt-6'>
        <div className='inline-flex basis-auto p-1 rounded-[8px] items-center gap-x-2 border border-gray-850 mb-4'>
          {annualList.slice(3, 6).map(item => {
            return (
              <FilterItem
                key={item.value}
                selected={current2 === item.value}
                itemData={item}
                onClick={item => setCurrent2(item)}
              />
            )
          })}
        </div>
        ...
        <div className='flex justify-center items-center gap-x-6'>
          <div className='flex items-center gap-x-1'>
            <div className='w-[11px] h-[11px] bg-[linear-gradient(180deg,rgba(190,255,110,0.5)_0%,rgba(106,252,223,0.5)_100%)] rounded-full'></div>
            <div className='text-[12px] font-normal'>{roeName2}</div>
          </div>
          {/* <div className="flex items-center gap-x-1">
              <div className="w-[11px] h-[11px] bg-[#76CEDF] rounded-full"></div>
              <div className="text-[12px] font-normal">Net income</div>
        ...
        <div className='text-[#9DA3AF] text-center text-xs/[15px] h-[15px] font-normal mt-4'>{roeFormula2}</div>
      </div>
      <div className='mt-6'>
        <div className='inline-flex basis-auto p-1 rounded-[8px] items-center gap-x-2 border border-gray-850 mb-4'>
          {annualList.slice(6).map(item => {
            return (
              <FilterItem
                key={item.value}
                selected={current3 === item.value}
                itemData={item}
                onClick={item => setCurrent3(item)}
              />
            )
          })}
        </div>
        ...
        <div className='flex justify-center items-center gap-x-6'>
          <div className='flex items-center gap-x-1'>
            <div className='w-[11px] h-[11px] bg-[linear-gradient(180deg,rgba(190,255,110,0.5)_0%,rgba(106,252,223,0.5)_100%)] rounded-full'></div>
            <div className='text-[12px] font-normal'>{roeName3}</div>
          </div>
          {/* <div className="flex items-center gap-x-1">
              <div className="w-[11px] h-[11px] bg-[#76CEDF] rounded-full"></div>
              <div className="text-[12px] font-normal">Net income</div>
        ...
        <div className='text-[#9DA3AF] text-center text-xs/[15px] h-[15px] font-normal mt-4'>{roeFormula3}</div>
      </div>
    </div>
  )
})
