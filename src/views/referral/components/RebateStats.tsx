...
function useRebateClaimState(isSignatureValid: boolean) {
  ...
  // const riskUserConfig = useKycStore(state => state.userConfig)
  // const kycStatus = useKycStore(state => state.kycStatus)

  // // 2-已通过 9-风控
  // const isKycFinished = kycStatus?.status === 2 || kycStatus?.status === 9

  // const needToFinishKyc = isKycFinished !== undefined ? !isKycFinished : undefined

  // const canClaimRebate =
  //   riskUserConfig !== undefined
  //     ? riskUserConfig?.actions !== undefined
  //       ? (riskUserConfig?.actions & (1 << 2)) !== 0
  //       : false
  //     : undefined
...
function OnRisk() {
  ...
  return (
    <IconWithTooltip
      ...
      triggerClassName='absolute top-[13px] right-[12px]'
      tooltip={
        <Trans
...
export default function RebateClaimButton(props: {
  ...
  return (
    <>
      <button
        className={`disabled:bg-[#1a1b1e] mt-4 disabled:border-none cursor-pointer h-[48px] rounded-[8px] w-full flex items-center justify-center disabled:cursor-not-allowed font-semibold text-[16px] disabled:text-[#737a87] bg-[#9CFF3A1A] border border-[#9CFF3A80] text-brand whitespace-nowrap`}
...
function ClaimRebateDialog(props: {
  ...
  return (
    <DialogController
      ...
      headerClassName={'border-b pt-4 text-left pb-3 px-6 border-b-gray-850'}
      ...
      className='w-[calc(100vw-48px)] max-w-[420px] top-[20vh] [@media(min-height:900px)]:top-[197px] bg-gray-950 border border-gray-850 rounded-2xl p-0 gap-0'
    >
...
function DataItem({ label, value, unit }: DataItemProps) {
  return (
    <div className='bg-[#1a1b1e] h-[60px] rounded-[8px] w-full flex items-center justify-between px-[16px] py-[8px]'>
      ...
      <div className='flex gap-[8px] ml-3 overflow-hidden items-baseline'>
