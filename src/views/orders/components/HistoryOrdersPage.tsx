...
export function SeeMoreOnWeb(props: { className?: string }) {
  ...
  return (
    ...
    >
      {'成交记录仅保留三个月内的成交记录，更多记录请访问WEB端查看，'}
      {TIKO_ORDER_URL}
...
const HistoryOrdersPage = ({ account, chainId }: { account: string; chainId: number }) => {
  const { t } = useTranslation()
  ...
  const {
    ...
    isFetchedAfterMount,
    refetch,
  } = useInfiniteQuery(infiniteOrderHistoryOptions(account, chainId, isSignatureValid))
  ...
  return (
    <div className='flex flex-1 flex-col px-5'>
      {isLoading ? (
        ...
        <>
          {orders.map(order => (
            <OrderCard key={order.orderId} order={order} from="history" />
          ))}
